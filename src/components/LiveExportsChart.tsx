    ...
    (async () => {
      try {
        const response = await fetch(
          'https://ec.europa.eu/eurostat/api/dissemination/statistics/1.0/data/nama_10_gdp?format=JSON&sinceTimePeriod=2010&untilTimePeriod=2024&geo=HU&geo=PL&geo=SK&geo=RO&na_item=P6&unit=PC_GDP'
        );
        ...
        if (mounted) setError('Failed to load exports data');
      }
    })();
    ...
    <Card className="bg-gradient-surface">
      <CardContent className="p-4">
        <div className="mb-3 flex items-start justify-between gap-4">
          <div>
            <h3 className="text-base font-semibold">Exports of goods & services (% of GDP)</h3>
            <p className="text-xs text-muted-foreground">Eurostat nama_10_gdp · % of GDP · na_item: P6 · unit: PC_GDP · 2010–2024</p>
          </div>
          <a className="story-link text-xs" target="_blank" rel="noreferrer" href="https://ec.europa.eu/eurostat/api/dissemination/statistics/1.0/data/nama_10_gdp?sinceTimePeriod=2010&untilTimePeriod=2024&geo=HU&geo=PL&geo=SK&geo=RO&na_item=P6&unit=PC_GDP&format=JSON">Live source</a>
        </div>
        {error && (
...
export default LiveExportsChart;
