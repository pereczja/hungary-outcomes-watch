    ...
    (async () => {
      try {
        const response = await fetch(
          'https://ec.europa.eu/eurostat/api/dissemination/statistics/1.0/data/demo_mlexpec?format=JSON&sinceTimePeriod=2010&untilTimePeriod=2024&geo=HU&geo=PL&geo=SK&geo=RO&sex=T&age=Y_LT1'
        );
        ...
        if (mounted) setError('Failed to load life expectancy data');
      }
    })();
    ...
    <Card className="bg-gradient-surface">
      <CardContent className="p-4">
        <div className="mb-3 flex items-start justify-between gap-4">
          <div>
            <h3 className="text-base font-semibold">Life expectancy at birth</h3>
            <p className="text-xs text-muted-foreground">Eurostat demo_mlexpec · years · sex: T · age: Y_LT1 · 2010–2024</p>
          </div>
          <a className="story-link text-xs" target="_blank" rel="noreferrer" href="https://ec.europa.eu/eurostat/api/dissemination/statistics/1.0/data/demo_mlexpec?sinceTimePeriod=2010&untilTimePeriod=2024&geo=HU&geo=PL&geo=SK&geo=RO&sex=T&age=Y_LT1&format=JSON">Live source</a>
        </div>
        {error && (
...
export default LiveLifeExpectancyChart;
