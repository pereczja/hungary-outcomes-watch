    ...
    (async () => {
      try {
        const response = await fetch(
          'https://ec.europa.eu/eurostat/api/dissemination/statistics/1.0/data/lfsi_emp_a?format=JSON&sinceTimePeriod=2010&untilTimePeriod=2024&geo=HU&geo=PL&geo=SK&geo=RO&sex=T&age=Y20-64&unit=PC_POP'
        );
        ...
        console.error('Error fetching employment rate data:', err);
        if (mounted) setError('Failed to load employment rate data');
      }
    })();
    ...
    <Card className="bg-gradient-surface">
      <CardContent className="p-4">
        <div className="mb-3 flex items-start justify-between gap-4">
          <div>
            <h3 className="text-base font-semibold">Employment rate (age 20–64)</h3>
            <p className="text-xs text-muted-foreground">Eurostat lfsi_emp_a · % of population · sex: T · age: Y20-64 · 2010–2024</p>
          </div>
          <a className="story-link text-xs" target="_blank" rel="noreferrer" href="https://ec.europa.eu/eurostat/api/dissemination/statistics/1.0/data/lfsi_emp_a?sinceTimePeriod=2010&untilTimePeriod=2024&geo=HU&geo=PL&geo=SK&geo=RO&sex=T&age=Y20-64&unit=PC_POP&format=JSON">Live source</a>
        </div>
        {error && (
...
export default LiveEmploymentRateChart;
