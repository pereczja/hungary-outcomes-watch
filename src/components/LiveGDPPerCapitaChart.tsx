...
const EUROSTAT_URL =
  'https://ec.europa.eu/eurostat/api/dissemination/statistics/1.0/data/nama_10_pc?format=JSON&sinceTimePeriod=2010&untilTimePeriod=2024&geo=HU&geo=PL&geo=SK&geo=RO&unit=CP_EUR_HAB&na_item=B1GQ';

export const LiveGDPPerCapitaChart = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // strides for decoding flat index
    ...
    <Card className="bg-gradient-surface">
      <CardContent className="p-4">
        <div className="mb-3 flex items-start justify-between gap-4">
          <div>
            <h3 className="text-base font-semibold">GDP per capita (EUR, nominal)</h3>
            <p className="text-xs text-muted-foreground">Eurostat nama_10_pc · unit: CP_EUR_HAB · na_item: B1GQ · 2010–2024</p>
          </div>
          <a className="story-link text-xs" target="_blank" rel="noreferrer" href="https://ec.europa.eu/eurostat/api/dissemination/statistics/1.0/data/nama_10_pc?sinceTimePeriod=2010&untilTimePeriod=2024&geo=HU&geo=PL&geo=SK&geo=RO&unit=CP_EUR_HAB&na_item=B1GQ&format=JSON">Live source</a>
        </div>
        {error && (
...
export default LiveGDPPerCapitaChart;
