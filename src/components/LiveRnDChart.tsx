    ...
    (async () => {
      try {
        const response = await fetch(
          'https://ec.europa.eu/eurostat/api/dissemination/statistics/1.0/data/rd_e_gerdtot?format=JSON&sinceTimePeriod=2010&untilTimePeriod=2024&geo=HU&geo=PL&geo=SK&geo=RO&unit=PC_GDP'
        );
        ...
        if (mounted) setError('Failed to load R&D expenditure data');
      }
    })();
    ...
    <Card className="bg-gradient-surface">
      <CardContent className="p-4">
        <div className="mb-3 flex items-start justify-between gap-4">
          <div>
            <h3 className="text-base font-semibold">R&D expenditure (% of GDP)</h3>
            <p className="text-xs text-muted-foreground">Eurostat rd_e_gerdtot · % of GDP · unit: PC_GDP · 2010–2024</p>
          </div>
          <a className="story-link text-xs" target="_blank" rel="noreferrer" href="https://ec.europa.eu/eurostat/api/dissemination/statistics/1.0/data/rd_e_gerdtot?sinceTimePeriod=2010&untilTimePeriod=2024&geo=HU&geo=PL&geo=SK&geo=RO&unit=PC_GDP&format=JSON">Live source</a>
        </div>
        {error && (
